...
function usingGateway(e){
	...
	var crif_error_msg = jQuery('#crif_error_msg').val();
    ...
    if(jQuery('form[name="checkout"] input[name="payment_method"]:checked').val() == 'bacs'){
		 jQuery(".woocommerce").prepend('<div class="woocommerce-error"><p>'+crif_error_msg+'</p></div>');		
		if(ccval) {		 	
		  jQuery(window).scrollTop(jQuery('.entry-title').offset().top);	
          jQuery("input[name='payment_method']:checked").prop( "checked", false ); 
